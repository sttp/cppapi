...
if errorlevel 1 exit /b 1
if not exist "%REPO%\build\tests\SubscriberStartupTest" mkdir "%REPO%\build\tests\SubscriberStartupTest"
pushd "%REPO%\build\tests\SubscriberStartupTest"
cl /nologo /std:c++latest /O2 /MD /EHsc /D_WIN32_WINNT=0x0601 /DBOOST_CONFIG_SUPPRESS_OUTDATED_MESSAGE /DUSE_UTF8_INSTEAD_OF_CODECVT /D_HAS_AUTO_PTR_ETC /DANTLR4CPP_STATIC /I"%STTP_BOOST_ROOT%" /Fe:SubscriberStartupTest.exe /Fo:SubscriberStartupTest.obj "%REPO%\src\samples\SubscriberStartupTest\SubscriberStartupTest.cpp" "%REPO%\build\output\x64\Release\lib\sttp.cpp.lib" /link /LTCG /LIBPATH:"%STTP_BOOST_ROOT%\stage\lib" ws2_32.lib
