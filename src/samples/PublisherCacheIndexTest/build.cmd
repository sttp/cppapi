...
if errorlevel 1 exit /b 1
if not exist "%REPO%\build\tests\PublisherCacheIndexTest" mkdir "%REPO%\build\tests\PublisherCacheIndexTest"
pushd "%REPO%\build\tests\PublisherCacheIndexTest"
cl /nologo /std:c++latest /O2 /MD /EHsc /D_WIN32_WINNT=0x0601 /DBOOST_CONFIG_SUPPRESS_OUTDATED_MESSAGE /DUSE_UTF8_INSTEAD_OF_CODECVT /D_HAS_AUTO_PTR_ETC /DANTLR4CPP_STATIC /I"%STTP_BOOST_ROOT%" /Fe:PublisherCacheIndexTest.exe /Fo:PublisherCacheIndexTest.obj "%REPO%\src\samples\PublisherCacheIndexTest\PublisherCacheIndexTest.cpp" "%REPO%\build\output\x64\Release\lib\sttp.cpp.lib" /link /LTCG /LIBPATH:"%STTP_BOOST_ROOT%\stage\lib" ws2_32.lib
