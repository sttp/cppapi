...
static std::vector<uint8_t> Metadata(const Shape& shape, size_t& xmlBytes)
{
    ...
    xmlBytes = text.size();
    if (!shape.gzip) return {text.begin(), text.end()};
...
class BenchmarkSubscriber final : public SubscriberInstance
{
public:
...
static void Validate(BenchmarkSubscriber& subscriber, const Shape& shape)
{
    ...
    subscriber.IterateConfigurationFrames([&](ConfigurationFramePtr frame, void*) {
        ...
        if (shape.kind == "phasor")
        {
            ...
            for (unsigned i = 1; i <= slots; ++i)
            {
                ...
                else Require(!phasor->Angle && !phasor->Magnitude && phasor->Phasor->Label == "UNDEFINED", "Wrong phasor placeholder");
            }
        }
        else
        {
            ...
            for (unsigned i = 1; i <= slots; ++i)
            {
                ...
                else Require(values[i - 1]->SignalID == Empty::Guid && values[i - 1]->PointTag == "UNDEFINED", "Wrong scalar placeholder");
            }
        }
    }, nullptr);
...
static void Run(const Shape& shape)
{
    ...
    const double elapsed = std::chrono::duration<double, std::milli>(BenchClock::now() - start).count();
    Validate(subscriber, shape);
    std::cout << "RESULT," << shape.kind << ',' << shape.devices << ',' << shape.points << ',' << shape.stride
              << ',' << shape.gzip << ',' << xmlBytes << ',' << payload.size() << ',' << elapsed << ",PASS" << std::endl;
}
...
int main(int argc, char** argv)
{
    try
    {
        if (argc == 2 && std::string(argv[1]) == "--regression")
        {
        ...
        if (argc < 4)
        {
            std::cout << "Usage: MetadataBenchmark analog|digital|phasor DEVICES POINTS_PER_DEVICE [STRIDE=1] [gzip]\n";
            return 1;
        }
        Shape shape{argv[1], static_cast<unsigned>(std::stoul(argv[2])), static_cast<unsigned>(std::stoul(argv[3])),
                    argc > 4 ? static_cast<unsigned>(std::stoul(argv[4])) : 1, argc > 5 && std::string(argv[5]) == "gzip"};
        Require(shape.kind == "analog" || shape.kind == "digital" || shape.kind == "phasor", "Invalid kind");
        Require(shape.devices > 0 && shape.points > 0 && shape.stride > 0, "Counts must be positive");
        Require(uint64_t(shape.points) * shape.stride <= std::numeric_limits<int32_t>::max(), "Index exceeds int32 range");
        Require(uint64_t(shape.devices) * shape.points * (shape.kind == "phasor" ? 2 : 1) <= 500000, "Limit this benchmark to 500,000 measurements");
        Require(uint64_t(shape.devices) * shape.points * shape.stride <= 1000000, "Limit total configuration slots to 1,000,000");
        Run(shape);
        return 0;
    }
